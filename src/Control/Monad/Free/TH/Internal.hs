...
-----------------------------------------------------------------------------
-- |
-- Module      :  Control.Monad.Trans.TH.Internal
...
--
-- Internal functions for generating free monadic actions. This module's
-- interface is subject to change.
--
----------------------------------------------------------------------------
module Control.Monad.Free.TH.Internal 
...
-- | Retrieves the types of those arguments which don't depend on the free
--   parameter.
params :: [Arg] -> [Type]
params [] = []
...
-- | Retrieves the types and associated expressions of those arguments which
--   capture the free parameter
...
-- | Get the name of a type-variable bound in a type expression.
tyVarBndrName :: TyVarBndr -> Name
tyVarBndrName (PlainTV  name)   = name
tyVarBndrName (KindedTV name _) = name

-- | Find a type in the current syntactic context.
findTypeOrFail :: String -> Q Name
findTypeOrFail s = lookupTypeName s >>= maybe (fail $ s ++ " is not in scope") return

-- | Find a value in the current syntactic context.
...
-- | Lifts a generalized constructor into a type declaration.
